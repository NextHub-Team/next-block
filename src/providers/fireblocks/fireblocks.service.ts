...
import {
  FIREBLOCKS_DEFAULT_ENV,
  SDK_VERSION,
} from './types/fireblocks-const.type';
import type { ConfigurationOptions } from '@fireblocks/ts-sdk/client/clientConfiguration';
...
import { basePathHelper } from './fireblocks.helper';
import { SecretConfigService } from 'src/secret/secret.service';
import * as path from 'path';

// TODO: Add cache for large data request like supportAssets , and pagination stream
@Injectable()
export class FireblocksService extends ConfigurableService<AllConfigType> {
  private fireblocks: Fireblocks;
  private readonly logger = new Logger(FireblocksService.name);

  @ConfigGetOrThrow('fireblocks.apiKeyAdmin', {
    envVarName: 'SECRET_FIREBLOCKS_API_KEY_ADMIN',
    inferEnvVar: true,
  })
  private readonly apiKeyAdmin: string;

  @ConfigGet('fireblocks.envMode', {
    inferEnvVar: true,
    defaultValue: FIREBLOCKS_DEFAULT_ENV,
  })
  ...
  constructor(
    ...
    @Inject('FIREBLOCKS_SECRET_PATH') private readonly secretsPath: string,
  ) {
    super(configService); // Pass the ConfigService to the base class
    this.logger.log('Initialized Fireblocks SDK Version:', SDK_VERSION);
    const apiSecretKey: string = this.getApiPrivateKey();
    // Initialize Fireblocks SDK
    this.fireblocks = new Fireblocks({
      apiKey: this.apiKeyAdmin,
      basePath: basePathHelper(this.envMode),
      secretKey: apiSecretKey,
    } as ConfigurationOptions);
  }

  // TODO: Improve output logs for paths
  private getApiPrivateKey(): string {
    const privateKeyFile = 'api_secret_private_key.pem';

    // Extract provider-specific relative path
    const providerRoot = 'providers'; // Define the root directory for providers
    const relativeSecretsPath = this.secretsPath.includes(providerRoot)
      ? this.secretsPath.split(providerRoot).pop()
      : path.relative(process.cwd(), this.secretsPath);
    ...
    const globalSecretPath = this.secretConfigService.get<string>(
      'GLOBAL_SECRET_PATH',
      { infer: true },
    );
    const relativeGlobalSecretsPath = globalSecretPath
      ? path.relative(process.cwd(), globalSecretPath)
      : 'Not defined';

    // Log paths
    this.logger.debug(
      `Secrets path for provider: ${relativeSecretsPath || 'Unknown'}`,
    );
    this.logger.debug(`Global secrets path: ${relativeGlobalSecretsPath}`);

    // Validate secretsPath
    if (!this.secretsPath) {
      this.logger.error('Secrets path is not defined.');
      throw new Error('Secrets path must be defined to fetch the private key.');
    }

    try {
      this.logger.debug(
        `Attempting to fetch private key from: ${relativeFilePath}`,
      );

      const privateKey = this.secretConfigService.getSecretFileContent(
        privateKeyFile,
        this.secretsPath,
      ) as string;
      ...
      if (!privateKey) {
        this.logger.error(
          `Retrieved private key is empty. Please check the content of: ${relativeFilePath}`,
        );
        throw new Error('Private key content is empty.');
      }

      this.logger.log(
        `Private key successfully retrieved from: ${relativeFilePath}`,
      );
      return privateKey;
    } catch (error) {
      if (error.code === 'ENOENT') {
        this.logger.error(
          `File not found: ${privateKeyFile} in path: ${relativeFilePath}`,
          error.stack,
        );
      } else if (error.code === 'EACCES') {
        this.logger.error(
          `Permission denied accessing file: ${relativeFilePath}`,
          error.stack,
        );
      } else {
        this.logger.error(
          `An unexpected error occurred while retrieving the private key from: ${relativeFilePath}`,
          error.stack,
        );
      }
      throw error;
  ...
  // }
    ...
    throw new Error(
      `[FireblocksService] Initialization failed: ${error.message}`,
    );
  }
  ...
  async createVaultAccountAsset(vaultAccountId: string, assetId: string) {
    try {
      return await this.fireblocks.vaults.createVaultAccountAsset({
        vaultAccountId,
        assetId,
      });
    } catch (error) {
  ...
  async createDepositAddress(
    ...
    try {
      return await this.fireblocks.vaults.createVaultAccountAssetAddress({
        assetId,
        vaultAccountId,
  ...
  async createTransaction(
    ...
    try {
      ...
      return await this.fireblocks.transactions.createTransaction({
