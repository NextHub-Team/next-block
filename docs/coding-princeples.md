...
- `*.type.ts` for type aliases (and for all files under `types/`)
- `*-enum.type.ts` for enums
- `*-const.type.ts` for constants
...
Types directory rule:

- Files in any `types/` folder must be named as `<domain>-<descriptor>.type.ts`.
- Encode the kind in the descriptor: `cmc-const.type.ts`, `api-gateway-enum.type.ts`, `queuedash-bullmq.type.ts`, `internal-events-index.type.ts`.
